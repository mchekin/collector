...
class Filesystem
{
    ...
    public function copy($source, $target)
    {
        ...
        foreach ($recursiveIteratorIterator as $file) {
            $targetPath = $target . DIRECTORY_SEPARATOR . $recursiveIteratorIterator->getSubPathName();
            if ($file->isDir()) {
    ...
    public function findShortestPath($from, $to, $directories = false)
    {
        ...
        if ('/' === $commonPath || 0 !== strpos($from, $commonPath) ) {
            return $to;
        }
    ...
    public function findShortestPathCode($from, $to, $directories = false, $staticCode = false)
    {
        ...
        $relativeTarget = substr($to, strlen($commonPath));

        $relativeTarget = $relativeTarget
            ? '.' . var_export('/' . $relativeTarget, true)
            : '';

        return $commonPathCode . $relativeTarget;
    }
